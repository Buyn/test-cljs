...
*** orders_test.cljs
:PROPERTIES:
:header-args: :tangle ./src/tests/orders_test.cljs
:END:
**** file orders_test
...
*** testhelper.cljs
:PROPERTIES:
:header-args: :tangle ./src/tests/testhelper.cljs
:END:
**** file orders_test
...
* TODO
** DONE подченить шадов кложур
*** DONE взять за основу
{
  "name": "Giggin",
  "version": "0.1.0",
  "description": "Giggin - find a gig!",
                   ...
                   [:div.item {:key id}
                    ...
                    [:div.action
                     ...
                     [:button.btn.btn--link.tooltip
                      ...
                      [:i.icon.icon--cross]]]]))]
#+end_src
**** DONE добавить панель
**** DONE добавит через мои перменные
**** DONE заменить функчию создания курсоров
чтоб она была только в стйте
**** CANCELED убрать остальные упоминания курсора
заменив регет на стейт с функцией
**** DONE убедится что работает
**** DONE подченить ф8 ф8 и ф8 ф7
***** org-mode <f8>
#+begin_src elisp
;;  --------------------------------------
...
***** (setq buf-scretch (buffer-name))

(setq buf-todo (buffer-name))
(select-frame-by-name buf-todo)
...
я обычно переключаю фреёмы по буферу в нём
(select-frame-by-name "giggin.org")
и всё работало хорошо пока не столкнулся с ошибкой
что он не находит буферы с имене созданых индерект буферов
(select-frame-by-name "giggin.org::TODO ")
притом что 
(buffer-name)
"giggin.org::TODO "
***** 
...
возврашет структуру а строку
Debugger entered--Lisp error: (wrong-type-argument listp #("TODO " 0 4 (fontified t face (org-todo org-level-1))))
  car(#("TODO " 0 4 (fontified t face (org-todo org-level-1))))
  eval((car (frame-parameter nil 'name)) nil)

(substring-no-properties (frame-parameter nil 'name))
...
(cdr (frame-parameter nil 'name))
q
(let* ((buf (get-buffer "giggin.org::TODO "))
       (win (get-buffer-window buf t)))
  (when win
    (select-frame-set-input-focus (window-frame win))))

**** DONE сделать тесты для заказов
***** DONE несправилось
(defn strip-components [hiccup]
  (cond
    (and (vector? hiccup) (fn? (first hiccup)))
...
авот с этим хикапом стрип не справился
[:div.item {:key nil} [:div.img [:img {:src nil, :alt nil}]] [:div.content [:p.title  × 1]] [:div.action [:div.price $0] [:button.btn.btn--link.tooltip {:data-tooltip Remove, :on-click #object[Function]} [:i.icon.icon--cross]]]]
это превратилось в 
оставив обжект
***** DONE расширить тест ордерс если есть ордер
посмотреть меняется ли вывод
***** DONE ошибка
****** 
***** DONE понять где считается сума убедится
что считается правельно 
сделав тест
***** CANCELED кажется вопрос сумой в том что сменился на курсор
и он обёрнут в вектор
***** CANCELED тест сумарной сумы
**** DONE понять как рабоатет into
особенно в этом примере
...
**** DONE поправить ошибку сумирования
**** DONE перенести стрип компонент
***** в отдельный класс
test-utils
hiccup-normalize
**** сделать тесты для админ понели
***** 
***** ии
довай сделаем тест если нужно то через мокинг
**** понять как работает панель
***** что делает
***** как устраена
***** SRC
****** file admin_panel.cljs
...
#+end_src
***** TODO Помоги разобратся как работает эта конструуция
***** да
(.set (db-ref path) value)
(.ref (database) (str/join "/" path))
да мне не доконца понятно как работает эта форма.
ну то есть я понимаю, что мы вызываем методы внутри обекта дато баз.
до это мы импортировали обект дато баз - по этому он не вызовет ошибку отсутвия символа.
а вот откуда компилятор знает что делать с сет и реф? мыже не довали ему таких символов?

**** сделать комит
*** file

(find-file-other-frame "~/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/22-subscribe/src/giggin/fb/db.cljs")

(find-file-other-frame 
"~/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/22-subscribe/src/giggin/components/admin_panel.cljs")


(find-file-other-frame "/home/buyn/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/22-subscribe/src/giggin/fb/init.cljs")

(find-file-other-frame "~/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/22-subscribe/src/giggin/components/orders.cljs")
...
diff -r 21-firebase-database 22-subscribe

#+end_src

#+RESULTS:
#+begin_example
Only in 21-firebase-database: package.json
Only in 22-subscribe/src/giggin/components: admin_panel.cljs
diff -r 21-firebase-database/src/giggin/components/orders.cljs 22-subscribe/src/giggin/components/orders.cljs
4c4,5
<             [giggin.components.checkout-modal :refer [checkout-modal]]))
---
>             [giggin.components.checkout-modal :refer [checkout-modal]]
...
>                       :on-click #(remove-from-order id)}
>                      [:i.icon.icon--cross]]]]))]
Only in 22-subscribe/src/giggin/fb: db.cljs
diff -r 21-firebase-database/src/giggin/fb/init.cljs 22-subscribe/src/giggin/fb/init.cljs
5c5
< 
---
>             
...
(defn firebase-init
  ...
  (firebase/initializeApp
   {:apiKey "your-api-key"
    ...
    :projectId "your-project-id"}))
***** хочу освоить
гугл firebase из кложур скрипта.
сдела такой фаил инициализации.
но несем понимаю как добится безопасности.
тоесть если я такой фаил полужу на сервер это будет не безопасно. поскольку любой кто получит доступ к файлу получается получит доступ к базеданых. Какие варианты сделать безапасно?
**** сделать комит
*** diff block
**** block
20-firebase-setup 19-ajax-transformations 
...
diff -r 20-firebase-setup 19-ajax-transformations 
#+begin_src eshell :dir /home/buyn/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/ :results output replace
diff -r 20-firebase-setup 21-firebase-database
...
#+begin_example
Only in 21-firebase-database: package.json
diff -r 20-firebase-setup/src/giggin/core.cljs 21-firebase-database/src/giggin/core.cljs
7c7,8
<             [giggin.api :as api]))
---
>             [giggin.api :as api]
...
>     (.getElementById js/document "app"))
>   (firebase-init))
Only in 21-firebase-database/src/giggin: fb
#+end_example
**** ranger
(ranger "/home/buyn/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/")
**** all
...
08-for-over-orders     13-checkout-modal      18-ajax-requests  23-sold-out              29-tips-and-tricks   
*** file
(find-file-other-frame "~/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/21-firebase-database/package.json")
(find-file-other-frame "/home/buyn/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/21-firebase-database/src/giggin/fb/init.cljs")
(find-file-other-frame "~/Dev/ClojureScript/tutorials/learn-reagent-course-files/increments/21-firebase-database/src/giggin/core.cljs")
